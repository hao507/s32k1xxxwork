...
void delay_(volatile int cycles)
{
	...
	do{
		...
		if(rLenNow > 0){
			for(int i=0; i<rLenNow; i++){
			 if(strncmp(mate_endOk,rData+i,strlen(mate_endOk)) == 0 ){
					sendStatus = endOkFind;
					mem909s_memrecvdataserver();
					uartReadIrqReset(UART1);
					return sendStatus;
				}
				else if(strncmp(mate_endError,rData+i,strlen(mate_endError)) == 0){
...
int mem909s_msmsendserver(char * AT_Order)
{
char mate_endOk[2]="OK";	
	char mate_endError[5]="ERROR";
	char rData[100]={0};
...
int mem909s_msmsenddataserver()
{
	char *AT_IPSENDEX="AT^IPSENDEX=1,1,\"7e010000000a00000000000a033131311203323232107e\"\n\r";
	int ret = -1;
	ret = mem909s_msmsendserver(AT_IPSENDEX);
	mem909s_memrecvdataserver();
	return ret;
}
char data[200]={0};
...
int mem909s_memrecvdataserver()
{
		...
		uartReadIrq(UART1,rData1+rDatanum,100,&rLenNow);
		if(rLenNow > 0){
			rDatanum+=rLenNow;
			uartReadIrqReset(UART1);
		}
	...
	/*	if(rLenNow > strlen(redata)){
			int i,data_num=0;
			for(i = 0;i < rLenNow;i++){
				if(strncmp(redata,rData+i,100) == 0 ){
					strncpy(data,rData+i,11);
					data_num = 11;
					for(int j = i;j < rLenNow;j++){
						sprintf(data+data_num,"%0.2x",rData[j]);
						data_num++;
					}
					uartReadIrqReset(UART1);
					uartReadIrq(UART1,rData,sizeof(rData),&rLenNow);
					if(rLenNow > 0){
						for(int j = 0;j < rLenNow;j++)
						sprintf(data+data_num+j,"%0.2x",rData[j]);
						return 1;
					}
				}
			}
...
int getReceivstatus()
{
	char datastart[8]="^IPDATA:";
	int step = -1;
	for(int i = 0;i < rDatanum;i++){
		if(strncmp(datastart,rData1+i,strlen(datastart)) == 0 ){
			if(data[i+15] == 0x00 && (data[i+15+99]== 0x63 || data[i+15+99]== 0x63)){
				return 1;
			}
		}
	}
...
{
	int ret = -1, timeout = 30, step = 0;
	char *AT_NDISDUP = "AT^NDISDUP=1,1\n\r";
	char *AT_IPINIT = "AT^IPINIT=\"cmnet\"\n\r";
	char *AT_IPOPEN = "AT^IPOPEN=2,\"TCP\",\"47.107.252.86\",10080\n\r";
	do{
	wdgTps_process();
		if(step == 0){
			if(mem909s_initSend(AT_NDISDUP) == endOkFind)
				step = 1;
				delay_(10000);
		}
		else if(step == 1){
			if(mem909s_initSend(AT_IPINIT) == endOkFind)
				step = 2;
		}
		else if(step == 2)
		{
				if(mem909s_initSend(AT_IPOPEN) == endOkFind)
					step = 3;
					ret = 0;
					break;
		}
...
int getLTE_start()
{
	...
	do{
		...
		if(rLenNow>0){
					...
					strncpy(rData,ltereadbuff,rLenNow);
					return sendStatus;
