...
int type0_send_type1_request()//type0 mcu与type1 can的发送函数 一样要设置send_type_1里的值
{
	...
	char data = 0x00;
	data = data | send_type_1.TYPE1_5EV;
	data = data | (send_type_1.TYPE1_VCC_12V_EN << 1);
	data = data | (send_type_1.TYPE1_TX2_SATA << 3);
	data = data | (send_type_1.TYPE1_TX2_NO_SATA << 4);
	send_data[3] = data;
...
int type0_send_type2()
{
	...
	char data = 0x00;
	data = data | send_type_2.TYPE2_ZYNQ;
	data = data | (send_type_2.TYPE2_VCC_12V_EN << 1);
	send_data[3] = data;
	Canbus1Send(send_ID, (unsigned char *)send_data, sizeof(send_data),CAN_STANDARD_MODE);
}
...
int receive_zynq_type0(char *power_rdata_type0,int power_rLenNow)//接收
{
	int flag = 0;
		...
		for(int i= 0;i < power_rLenNow;i++){
			if(power_rdata_type0[i] == receive_HADE_0){
				...
				if(power_rdata_type0[type0_data_strat+2] == 0x01){
						...
						Analysis_zynq_type0(0x01);//如果cmd为1 保存电源状态	
						uartReadIrqReset(UART0);
					}
				if(power_rdata_type0[type0_data_strat+2] == 0x03){
						Analysis_zynq_type0(0x03);
						uartReadIrqReset(UART0);
				}
				}
			}
...
*/
	...
	char send_data[10]={0};
	char send_num = 0x00;
	send_data[send_num++] = SEND_HADE_0;
	send_data[send_num++] = SEND_HADE_1;
	send_data[send_num++] = 0x01;
	send_data[send_num++] = send_num_FPGA++;;
	send_data[send_num++] = 0x03;
	send_data[send_num++] = send_type_0.switch_flag;
	char data = 0x00;
	...
	send_data[send_num++] = 0x00;
	crt = send_data[0] ^ send_data[1];
	for(int i = 2; i < send_num;i++){
		crt = crt ^ send_data[i];
	}
	send_data[send_num++] = crt;
	send_data[send_num++] = 0x0d;
	int sLen;
...
int Analysis_zynq_type0(int cmd )//解析 并操作电源
{
	...
	for(int i = 2;i < type0_data_end-type0_data_strat-1;i++){
		crt = crt ^ data_buf[i];
	}
	...
	if(cmd == 0x03){
		...
		if(data_buf[5] == ON && send_type_0.switch_flag == ON){
				if((data_buf[6] & 1) && send_type_0.TYPE0_ZYNQ  == 1){
					Save_state_type0.TYPE0_ZYNQ = 1;
					Execution_TYPE0_ZYNQ(ON);
				}
				 if((data_buf[6] >> 1 & 1) && send_type_0.TYPE0_VCC_12V_EN == 1){
					Save_state_type0.TYPE0_VCC_12V_EN = 1;
					Execution_TYPE0_TYPE0_VCC_12V_EN(ON);
				}
				 if((data_buf[6]  >> 2 & 1) && send_type_0.TYPE0_4G == 1){
					Save_state_type0.TYPE0_4G = 1;
					Execution_TYPE0_TYPE0_4G(ON);
				}
					
			}
			else if(data_buf[5] == OFF && send_type_0.switch_flag == OFF){
				if((data_buf[6] & 1) && send_type_0.TYPE0_ZYNQ  == 1){
					Save_state_type0.TYPE0_ZYNQ = 0;
					Execution_TYPE0_ZYNQ(OFF);
				}
				 if((data_buf[6] >> 1 & 1) && send_type_0.TYPE0_VCC_12V_EN == 1){
					Save_state_type0.TYPE0_VCC_12V_EN = 0;
					Execution_TYPE0_TYPE0_VCC_12V_EN(OFF);
				}
				 if(data_buf[6]  >> 2 & 1){
					Save_state_type0.TYPE0_4G = 0;
					Execution_TYPE0_TYPE0_4G(OFF);
				}
			}
	}
...
int Execution_TYPE0_ZYNQ(int val)
{
	if(val == ON){
...
int Execution_TYPE0_TYPE0_VCC_12V_EN(int val)
{
	if(val == ON){
...
int Execution_TYPE0_TYPE0_4G(int val)
{
	if(val == ON){
		GpioSetVal(PORT_D, 16, PORT_HIGH);
	}
	else if(val == OFF){
		GpioSetVal(PORT_D, 16, PORT_LOW);
	}
}
