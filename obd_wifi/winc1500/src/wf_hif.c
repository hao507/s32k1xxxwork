 ...
 uint16_t u16DataOffset)
{
	...
	hif.opCode		= u8Opcode&(~NBIT7);
	hif.groupId		= groupId;
	hif.length	= M2M_HIF_HDR_OFFSET;
	if(pu8DataBuf != NULL)
	{
		hif.length += u16DataOffset + u16DataSize;
	}
	else
	{
		hif.length += u16CtrlBufSize;
	}
    if (hif.length <= M2M_HIF_MAX_PACKET_SIZE)
    {
	ChipWake();
	if(ret == M2M_SUCCESS)
	{
...
/*please define in firmware also*/
		...
		reg |= (groupId == M2M_REQ_GROUP_IP) ? (NBIT3):(0); /*IP = 1 or non IP*/
		reg |= ((uint32_t)hif.length << 4); /*length of pkt max = 4096*/
		...
		for(cnt = 0; cnt < 1000; cnt ++)
		{
			nm_read_reg(WIFI_HOST_RCV_CTRL_2);
			...
			if(cnt >= 500) {
				if(cnt < 501) {
					dprintf("Slowing down...\n");
				}
				nm_bsp_sleep(1);
			}
			if (!(reg & NBIT1))
			{
				nm_read_reg(WIFI_HOST_RCV_CTRL_4);
				if(ret != M2M_SUCCESS) {
		...
		if (dma_addr != 0)
		{
			...
			hif.length=hif.length;
			nm_write_block(u32CurrAddr, (unsigned char*)&hif, M2M_HIF_HDR_OFFSET);
			if(M2M_SUCCESS != ret) goto ERR1;
		...
		{
			 ChipSleep();
			dprintf("Failed to alloc rx size %d\r",ret);
			ret = M2M_ERR_MEM_ALLOC;
			goto ERR2;
		}
	}
	...
	{
            dprintf("(HIF)Failed to wakeup the chip\n");
            goto ERR2;
        }
	}
	else
	{
        dprintf("HIF message length (%d) exceeds max length (%d)\n",hif.length, M2M_HIF_MAX_PACKET_SIZE);
        ret = -1;
		goto ERR2;
	}
	/*actual sleep ret = M2M_SUCCESS*/
