...
static unsigned int timeLpitMs = 0;
//--------------------------------------
...
void LPIT0_init (void) // 1ms
{
  ...
  S32_NVIC->ICPR[(uint32_t)(LPIT0_Ch0_IRQn) >> 5U] = (uint32_t)(1UL << ((uint32_t)(LPIT0_Ch0_IRQn) & (uint32_t)0x1FU));
...
void timeLpitMsGet(unsigned int* ms){
	*ms = timeLpitMs;
}
