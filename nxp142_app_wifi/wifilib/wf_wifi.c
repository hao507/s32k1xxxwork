...
#if defined(M2M_ENABLE_SOFT_AP_MODE)
int m2m_wifi_enable_ap(const tstrM2MAPConfig* pstrM2MAPConfig)
{
#if defined(M2M_ENABLE_ERROR_CHECKING)  
...
#endif    
   int ret =  hif_send(M2M_REQ_GROUP_WIFI, AP_REQ_ENABLE_AP, (uint8_t *)pstrM2MAPConfig, sizeof(tstrM2MAPConfig), NULL, 0, 0);    
		return ret;
}

int  m2m_wifi_disable_ap(void)
{
   int ret =  hif_send(M2M_REQ_GROUP_WIFI, AP_REQ_DISABLE_AP, NULL, 0, NULL, 0, 0);
	return ret;
}
#endif // M2M_ENABLE_SOFT_AP_MODE
...
void m2mStub_PinSet_SPI_SS(t_m2mWifiPinAction action)
{
	if (action == M2M_WIFI_PIN_LOW)
	{
		GpioSetVal(PORT_B,5,PORT_LOW);
	}
	else
	{
		GpioSetVal(PORT_B,5,PORT_HIGH);
	}
}
#include <string.h>
#include "uart.h"
...
{
if(txLen == 0){
	uartReadIrq(UART1,p_rxBuf,rxLen,&rlen);
	if(rlen == rxLen)
		return;
}
else if(rxLen == 0){
		uartSendIrq(UART1,p_txBuf,txLen,&slen);
	if(rlen == txLen)
		return ;
}
uartReadIrq(UART1,p_rxBuf,rxLen,&rlen);
...
//	int timeout = 10000;
//	if(txLen == 0){
////			SpiSendIrq(buf,rxLen);
//		 spiReadIrq(p_rxBuf,rxLen);
////		while(1){
////			timeout--;
////			if(timeout == 0){
////				 spiReadIrq(p_rxBuf,rxLen);
////					timeout = 10000;
////			}
////			retread = spi0ReadStatusGet();
////			if(retread == SPI_READ_OVER)
////				return;
////		}
//		return;
//	}
//	else if(rxLen == 0){
//		SpiSendIrq(p_txBuf,txLen);
////		spiReadIrq(buf,txLen);
//		while(1){
//			timeout--;
//			if(timeout == 0){
//				 SpiSendIrq(p_txBuf,txLen);
//					timeout = 10000;
//			}
//			retsend = spi0SendStatusGet();
//			if(retsend == SPI_SEND_OVER)
//				return;
//		}
//	}
//	if(txLen > rxLen){
...
//			retread = spi0ReadStatusGet();
//			retsend = spi0SendStatusGet();
//			if(retread == SPI_READ_OVER && retsend == SPI_SEND_OVER)
//				return;
//		}
//	}
//	else if(txLen < rxLen){
//		strncat(( char *)p_txBuf,buf,rxLen-txLen);
//		SpiSendIrq(p_txBuf,rxLen);
//		spiReadIrq(p_rxBuf,rxLen);
//		while(1){
//				timeout--;
//			if(timeout == 0){
...
{
	return TimerLpitCreate(1); //function added to MCC generated timer1 code
}
...
void m2m_wifi_handle_events(t_m2mWifiEventType eventCode,
t_wifiEventData *p_eventData)
{
	if(eventCode == M2M_WIFI_DRIVER_INIT_EVENT){
		int b;
	}
	else if(eventCode == M2M_WIFI_IP_CONFLICT_EVENT){
	}
	else if(eventCode == M2M_WIFI_RSSI_EVENT){
	}
	else if(eventCode == M2M_WIFI_SCAN_DONE_EVENT){
	}
	else if(eventCode == M2M_WIFI_SCAN_RESULT_EVENT){
	}
	else if(eventCode == M2M_WIFI_CONN_STATE_CHANGED_EVENT){
	}
	else if(eventCode == M2M_WIFI_IP_ADDRESS_ASSIGNED_EVENT){
	}
	else if(eventCode == M2M_WIFI_SYS_TIME_EVENT){
	}
	else if(eventCode == M2M_WIFI_CONN_INFO_RESPONSE_EVENT){
	}
	else if(eventCode == M2M_WIFI_WPS_EVENT){
	}
	else if(eventCode == M2M_WIFI_PROVISION_INFO_EVENT){
	}
	else if(eventCode == M2M_WIFI_DEFAULT_CONNNECT_EVENT){
	}
	else if(eventCode == M2M_WIFI_PRNG_EVENT){
	}
}
...
{
	if(eventCode == M2M_SOCKET_BIND_EVENT){
	}
}
