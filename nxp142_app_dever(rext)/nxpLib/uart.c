...
//		printf("uart baudrate err: %d...\n", baudrate);
	...
	if(port == UART0){
		LPUART0->BAUD = LPUART_BAUD_SBR(sbr)
						|LPUART_BAUD_OSR(osr)| LPUART_BAUD_SBNS(1);
		...
		LPUART1->BAUD = LPUART_BAUD_SBR(sbr)  	/* Initialize for 9600 baud, 1 stop: */
								|LPUART_BAUD_OSR(osr) | LPUART_BAUD_SBNS(1);  	/* SBR=52 (0x34): baud divisor = 8M/9600/16 = ~52 */
...
//-------------------------------------------------------------
void uartReadIrqReset(uint8_t port){
	if(UART0 == port){
    	uart0_readNum = uart0_needReadNum = 0;
    	uart0_readStatus = UART0_READ_OVER;
			memset(uart0_recv, 0, sizeof(uart0_recv));
	}
	else if(UART1 == port){
