...
unsigned char wdata[8] = {0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08};
	...
	{
		Canbus1Send(TYPE2_Can_Send_Id_1,(unsigned char *)wdata,8,CAN_STANDARD_MODE);
	}
	int queueLen;
	if(TYPE2_Canread_statust0 == 1 && TYPE2_send_flag_1 == 1)
	{			
		int ret = canbus0ReadIrq((int *)&TYPE2_TYPE2_Can_read_Id,TYPE2_Read_data0,&queueLen);
	...
	{
		Canbus0Send(TYPE2_Can_Send_Id_0,(unsigned char *)wdata,8,CAN_STANDARD_MODE);
	}
 if(TYPE2_Canread_statust1 == 1 && TYPE2_send_flag_0 == 1){
	 int ret1 = canbus1ReadIrq((int *)&TYPE2_TYPE2_Can_read_Id,TYPE2_Read_data1,&queueLen);
