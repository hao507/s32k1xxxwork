...
int type0_send_type1_request()//type0 mcu与type1 can的发送函数 一样要设置send_type_1里的值
{
  ...
  Canbus0Send(send_ID, (unsigned char *)send_data, sizeof(send_data),CAN_STANDARD_MODE);
}
...
int receive_zynq_type0()//接收
{
	int power_rLenNow;
	int flag = 0;
	uartReadIrq(UART0,power_rdata_type0,sizeof(power_rdata_type0),&power_rLenNow);
		...
		if(flag == 2){
			...
			else if(power_rdata_type0[type0_data_strat+2] == 0x02){
...
*/
	...
	send_data[9] = 0x0d;
	int sLen;
	int sret =  uartSendIrq(UART0,(unsigned char *)send_data,sizeof (send_data),&sLen);
	if(sret == UART1_SEND_OVER){
				 uartSendIrq(UART0,(unsigned char *)send_data,sizeof (send_data),&sLen);
	}
...
int Analysis_zynq_type0()//解析关闭电源是否允许关闭
{
	...
	if(crt != data_buf[type0_data_end-1]){
		return -1;
	}
...
int send_data_Power_list_type0()//发送电源列表
{
	...
	data0 = data0 | Save_state_type0.TYPE0_ZYNQ;
	data0 = data0 | Save_state_type0.TYPE0_VCC_12V_EN << 1;
	data0 = data0 | Save_state_type0.TYPE0_4G << 2;
	...
	send_data[12] = 0x0d;
        int rLenNow;
        int sret = uartSendIrq(UART0,(unsigned char *)send_data,sizeof(send_data),&rLenNow);
        if(sret == UART0_SEND_OVER){
                uartSendIrq(UART0,(unsigned char *)send_data,sizeof(send_data),&rLenNow);
        }
}
