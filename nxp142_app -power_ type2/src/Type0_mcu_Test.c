...
#include "wdgTps.h"
#include "mem909s.h"
//HEAD		ID			数据长度	data		data		ROLLCNT  CRT  	 END4
...
{	
	...
	int flag_1 = -1,flag_2 = 0,flag_3 = 0;
	unsigned char ID;
	...
	for(int i = 0; i < 200;i++)
	{
		if(rData[i] == Read_Head_0 && rData[i+1] == Read_Head_1 && flag_1 == -1)
		{
			flag_1 = 0;
			ID_Start = i+2;
			if(rData[ID_Start+1] == 0X00){//data为0时
					crt = rData[ID_Start-2]^rData[ID_Start-1];
					for(j = 0;j < 3;j++)
						{
							crt = crt ^ rData[ID_Start+j];
						}
					if(rData[ID_Start+3] == crt)
						{
							flag_1 = 1;
						}
					else{
					...
					if(rData[ID_Start+2] == Read_ROLLCNT && flag_1 == 1)
						{
							flag_2 = 1;
							flag_1 = 2;
						}
				...
				if(rData[ID_Start+1] == 0X02) //len = 2
				{
						crt = rData[ID_Start-2]^rData[ID_Start-1];
						for(j = 0;j < 5;j++)
						{
					...
					if(rData[ID_Start+4] == Read_ROLLCNT && flag_1 == 1)
						{
								flag_2 = 1;
						}
					else
...
int type0_MCU_test_Canbus()
{
	...
	if(flag_0 == 1 && flag_1 != 1)
		...
		type0_MCU_test_Send(2,12,send_buf);
	else if(flag_0 == 1 && flag_1 == 1)//两个都发送
		type0_MCU_test_Send(2,24,send_buf);
	if(Canread_statust0 != 0 || Canread_statust1 != 0){//如果有一个接收就调用接收函数
		Can_read();
	}
		return 0;
}
...
//HEAD		ID			数据长度	data	ROLLCNT  CRT  	 END4
//0X55 66	1BYTE		1BYTE		   			 0~255	          0X0D
// 0    1  2        3        4       5      6         7     
