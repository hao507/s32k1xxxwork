...
int Can_Send_Id_0 = 0x123;
int Can_Send_Id_1 = 0x234;
int send_flag_0 = 0;
int send_flag_1 = 0;
...
{
	if(send_flag_1 == 1){
		CanbusSend(CANBUS1,Can_Send_Id_1,(unsigned char *)wdata,8,CAN_STANDARD_MODE);
	}


	int queueLen;
	if(Canread_statust0 == 1 && send_flag_1 == 1)
	{			
		canbusReadIrq(CANBUS0,(int *)&Can_Read_Id,Read_data0,&queueLen);
				if((strlen((char *)Read_data0) > 0 ) && Can_Read_Id!= Can_Send_Id_0){
					unsigned char *pa;
	...
	if(send_flag_0 == 1){
		CanbusSend(CANBUS0,Can_Send_Id_0,(unsigned char *)wdata,8,CAN_STANDARD_MODE);
	}

 if(Canread_statust1 == 1 && send_flag_0 == 1){
	 canbusReadIrq(CANBUS1,(int *)&Can_Read_Id,Read_data1,&queueLen);
					if((strlen((char *)Read_data1) > 0) && Can_Read_Id!= Can_Send_Id_1){
						unsigned char *pa;
						unsigned char tab[12];
						pa = (unsigned char *)&Can_Read_Id;
						tab[0]=*((char *)pa+0);
		...
		if(Canread_statust0 == 1 && Canread_statust1 == 1 && send_flag_0 == 0 && send_flag_1 == 0){
			return 0;
		}
		else if(flag_can == 1 && send_flag_0==0) {
			type0_MCU_test_Send(2,12,buf);
			flag_can = 0;
			Canread_statust0 =0;
			send_flag_1 = 0;
		}
		else if(flag_can == 2 && send_flag_1 == 0){
			type0_MCU_test_Send(2,12,buf);
			flag_can = 0;
			Canread_statust1 = 0;
			send_flag_0 = 1;
		}
		else if(flag_can == 3  && send_flag_0==1 && send_flag_1 == 1){
		...
		else if(flag_can == 1 && Canread_statust1 == 0){
			type0_MCU_test_Send(2,12,buf);
			flag_can = 0;
			Canread_statust1 =0;
			send_flag_1 = 0;
		}
		else if(flag_can == 2 && Canread_statust0 == 0){
			type0_MCU_test_Send(2,12,buf);
			flag_can = 0;
			Canread_statust1 =0;
			send_flag_0 = 0;
